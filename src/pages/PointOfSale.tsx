...
export default function PointOfSale() {
  ...
  return (
    // FIXED: Use h-full to fit perfectly into App shell
    <div className="h-full flex flex-col md:flex-row bg-slate-950 text-white relative font-sans">
      ...
      <div className="flex-1 flex flex-col h-full min-h-0">
        <div className="p-3 md:p-4 border-b border-slate-800 flex flex-col gap-3 bg-slate-900/50 z-10 shrink-0">
          ...
          <div className="relative bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 flex items-center gap-3 w-full md:w-fit hover:border-emerald-500/50 transition-colors cursor-pointer">
            <div className="bg-emerald-500/10 p-1.5 rounded-lg pointer-events-none"><User size={16} className="text-emerald-400" /></div>
            ...
            <select className="absolute inset-0 w-full h-full opacity-0 cursor-pointer appearance-none z-10" value={operator} onChange={(e) => setOperator(e.target.value)}>
          ...
          <div className="relative w-full">
            ...
            <input className="w-full bg-slate-900 border border-slate-700 rounded-xl py-3 pl-10 pr-4 text-white focus:ring-2 focus:ring-emerald-500 outline-none placeholder:text-slate-600 transition-all" placeholder="Search item code or name..." value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 pb-24 md:pb-4 md:p-4 bg-slate-950">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 md:gap-3">
            {filteredItems.map(item => (
              <button key={item.id} onClick={() => addToCart(item)} className="bg-slate-900/50 border border-slate-800 hover:border-emerald-500/50 active:bg-slate-800 p-3 md:p-4 rounded-xl flex flex-col items-start text-left transition-all active:scale-[0.98] group relative overflow-hidden">
                <div className="flex justify-between w-full mb-2">
                  ...
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${item.quantity > 0 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>{item.quantity} LEFT</span>
                </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-800 p-3 z-40 safe-area-bottom">
        <button onClick={() => setShowCartMobile(true)} className="w-full bg-emerald-500 hover:bg-emerald-400 text-slate-950 rounded-xl p-1 flex items-center justify-between shadow-lg shadow-emerald-500/20 active:scale-[0.99] transition-transform">
      ...
      <div className={`fixed inset-0 z-50 bg-slate-950 flex flex-col transition-transform duration-300 ease-out md:relative md:inset-auto md:w-[380px] md:translate-y-0 md:border-l md:border-slate-800 md:shadow-2xl ${showCartMobile ? 'translate-y-0' : 'translate-y-full md:translate-y-0'}`}>
        <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900 shrink-0">
          ...
          <button onClick={() => setShowCartMobile(false)} className="md:hidden p-2 bg-slate-800 rounded-full hover:bg-slate-700 active:bg-slate-600 text-slate-400"><ChevronDown size={24} /></button>
        </div>
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-950 min-h-0">
          {cart.length === 0 ? <div className="flex flex-col items-center justify-center h-full text-slate-600 gap-4"><Package size={48} className="opacity-20" /><p className="font-medium">Cart is empty</p></div> : cart.map(item => (
            <div key={item.id} className="bg-slate-900/50 p-3 rounded-xl border border-slate-800/50 flex gap-3">
              ...
              <div className="flex flex-col items-end justify-between py-1"><div className="font-mono font-bold text-emerald-400">RM {item.customPrice * item.qty}</div><button onClick={() => removeFromCart(item.id)} className="text-slate-600 hover:text-rose-400 transition-colors"><Trash size={16}/></button></div>
            </div>
          ))}
        </div>
        <div className="p-4 bg-slate-900 border-t border-slate-800 safe-area-bottom shrink-0">
          ...
          <button onClick={checkout} disabled={cart.length === 0} className="w-full bg-emerald-500 hover:bg-emerald-400 disabled:bg-slate-800 disabled:text-slate-600 text-slate-950 font-black py-4 rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 active:scale-[0.98] transition-all"><CreditCard size={20} /> CHARGE</button>
        </div>
      </div>
    </div>
  );
